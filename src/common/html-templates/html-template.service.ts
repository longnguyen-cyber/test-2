...
import { HtmlTemplateOptions, EMAIL_TEMPLATE_OPTIONS } from './html-template.const';
...
export class HtmlTemplateService {
  ...
  constructor(@Inject(EMAIL_TEMPLATE_OPTIONS) private readonly options: HtmlTemplateOptions) {
    this.email = new Email({
      ...
      getPath: (
        ...
        return path.join(this.options.templateDir, template, `${filename}.${language}`);
      },
      juiceResources: {
        webResources: {
