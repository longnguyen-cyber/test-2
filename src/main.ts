...
import { description as pkgDesc, name as pkgName, version as pkgVersion } from '../package.json';
...
async function bootstrap(): Promise<void> {
  ...
  app.use('/apidoc/', forwardedPrefixSwagger, swaggerUi.serve, swaggerUi.setup(document));
  app.use('/apidoc-json/', forwardedPrefixSwagger, (req: Request, res: Response) => {
  ...
  app.useGlobalPipes(
    new ValidationPipe({
      whitelist: true,
      transform: true,
      exceptionFactory: (errors: ValidationError[]): SeimBadRequestException => {
  ...
  app.get(WINSTON_MODULE_NEST_PROVIDER).log(`Swagger is running on: ${await app.getUrl()}/apidoc`);
}
